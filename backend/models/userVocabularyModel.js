...
const userVocabularyModel = {
    ...
    async add(userId, userVocabulary, currentLanguageId) {
        const values = [];
        ...
        // Add filter param
        values.push(currentLanguageId);
        const query = `
            WITH inserted AS (
            INSERT INTO user_vocabulary
                (user_id, word_id, language_id, mastery_level, last_review, created_at)
            VALUES ${placeholders}
            RETURNING *
            )
            SELECT 
                ...
                created_at
            FROM inserted
            WHERE language_id = $${values.length};
        `;
        const result = await pool.query(query, values);
        // reshape → { wordId: { language_id, mastery_level, last_review, created_at } }
    ...
    async get(userId, learning_language_id) {
        const query = `
        SELECT
            word_id,
            language_id,
