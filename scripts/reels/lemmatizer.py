...
class LemmaToken(NamedTuple):
    ...
    # Farsi only - "مادر من" for "مادرم", when hazm safely reduced the
    # token to a root by stripping what looks like a possessive suffix.
    # Not used for dictionary matching (that's `lemma`) - stored as an
    # extra display-only field merged into dialogues.sentences_json.
...
# Persian possessive enclitics, mapped to the pronoun they stand for.
# Includes both spellings (formal تان/شان/مان and colloquial تون/شون/مون)
# and the alef-buffered variants used after a vowel-ending root (e.g.
# "خانه" + "ات" -> "خانه‌ات", not "خانهت") - since the suffix is detected
# by diffing the ORIGINAL token against hazm's root, not the informal-
# normalized intermediate, both spellings can appear literally.
_POSSESSIVE_SUFFIX_TO_PRONOUN = {
    "شان": "آنها", "مان": "ما", "تان": "شما",
    "شون": "آنها", "مون": "ما", "تون": "شما",
    "اشان": "آنها", "امان": "ما", "اتان": "شما",
    "اشون": "آنها", "امون": "ما", "اتون": "شما",
    "ش": "او", "م": "من", "ت": "تو",
    "اش": "او", "ام": "من", "ات": "تو",
}


def _detect_possessive_expansion(token: str, root: str) -> Optional[str]:
    """If hazm safely reduced `token` to `root` (validated against hazm's
    own dictionary - see _resolve_hazm_lemma) by stripping what is
    specifically a possessive suffix, returns the expanded "root pronoun"
    form (e.g. "مادر من" for "مادرم"). Returns None if `root` isn't a
    prefix of `token`, or the stripped part isn't one of the known
    possessive suffixes - hazm's stemmer strips other suffix types too
    (plurals, comparatives, ...), which this must not misidentify as
    possessive.
    """
    if root == token or not token.startswith(root):
        return None
    removed = token[len(root):].lstrip("‌")
    pronoun = _POSSESSIVE_SUFFIX_TO_PRONOUN.get(removed)
    return f"{root} {pronoun}" if pronoun else None
    ...
    if language_code == "fa":
        ...
        for tok in tokens:
            ...
            if "#" in raw_lemma:
                ...
                lemma = raw_lemma
                pos = None
                # Must run on hazm's raw (undiacritized-but-not-yet-lowered)
                # output, before strip_diacritics/lower below - it does
                # exact prefix matching against what hazm actually returned.
                expanded = _detect_possessive_expansion(tok, raw_lemma)
            result.append(LemmaToken(
                lemma=strip_diacritics(lemma).lower(),
                part_of_speech=pos,
                expanded=expanded,
            ))
        return result
